...
function modifierText(value){
 const text=String(value??'').trim(),colon=text.indexOf(':');
 if(colon<0)return text;
 const group=text.slice(0,colon).trim(),selection=text.slice(colon+1).trim();
 // Instruction prefixes carry meaning; group headings do not. Never strip allergies.
 if(/\bALLERG(?:Y|IC|EN)\b/i.test(group))return text;
 if(/^(?:NO|EXTRA|WITHOUT|ADD|SIN|CON|LIGHT|LESS|ON THE SIDE)$/i.test(group)){
  return selection.toUpperCase().startsWith(group.toUpperCase()+' ')?selection:text;
 }
 return selection;
}
function modifier(m){const text=modifierText(m);if(!text)return '';const special=/\b(?:ALLERG(?:Y|IC|EN)|NO|EXTRA|WITHOUT)\b/i.test(text);return '<div class="modifier '+(special?'important':'')+'">'+esc(text)+'</div>';}
...
function render(force=false){
 const store=active(),orders=store.orders().filter(isVisible),count=config.columns*config.rows;
 page=0;
 ...
 const scroll=new Map([...$('board').querySelectorAll('.ticket')].map(el=>[el.dataset.ticket,el.querySelector('.ticket-body').scrollTop]));
 ...
 $('board').querySelectorAll('.ticket').forEach(el=>el.querySelector('.ticket-body').scrollTop=scroll.get(el.dataset.ticket)||0);
 ...
 $('pages').textContent=`${page+1} / ${Math.max(1,Math.ceil(orders.length/count))}`;$('previous').disabled=page===0;$('next').disabled=(page+1)*count>=orders.length;
 ...
 $('health').textContent=demo?'Demo display · live receiver stays on':config.station==='expo'?'Expo station':`${status.network===false||status.listening===false?'Receiver needs attention':'Receiver ready'} · ${status.ip}${status.port?':'+status.port:''} · UDP 3289${status.receiverStatus?' · '+status.receiverStatus:''}`;
...
document.addEventListener('click',async e=>{
 ...
 if(b.dataset.view){station=b.dataset.view;page=0;$('board').scrollLeft=0;render(true);return;}
...
$('history').onclick=()=>{history=!history;page=0;$('board').scrollLeft=0;render(true);};$('previous').onclick=()=>{page--;render(true);};$('next').onclick=()=>{page++;render(true);};
